...
| Node-development guidance also relevant to `housegraph-nodes` / `housegraph-plugin-template` (rules for out-of-tree node authors, control-vs-action design) | [`docs/architecture/nodes.md`](docs/architecture/nodes.md) and [`docs/architecture/plugins.md`](docs/architecture/plugins.md) — this repo is the source of truth; the companion repos link here rather than duplicating the text, so there is nothing to sync |
