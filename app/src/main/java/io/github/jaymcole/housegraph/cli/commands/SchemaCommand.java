...
public final class SchemaCommand implements Command {
    ...
    private static final Map<String, String> RESOURCES = Map.of(
            "graph", "/schema/graph-save.v6.schema.json",
            "graph-v6", "/schema/graph-save.v6.schema.json",
    ...
    @Override
    public String usage() {
        return "  schema [graph|graph-v6|graph-v5|graph-v4|graph-v3|graph-v2|catalog]\n\n"
                + "graph (default) is the JSON Schema for the save-file format this build writes\n"
                + "(version 6). graph-v6 … graph-v2 name a specific format version, for checking a\n"
                + "file this build did not write. catalog is the schema for\n"
    ...
    public int run(Args args) {
        ...
        if (resource == null) {
            out.println("Unknown schema: " + which
                    + " (expected graph, graph-v6, graph-v5, graph-v4, graph-v3, graph-v2 or catalog)");
            out.println(usage());
            return 2;
        }
