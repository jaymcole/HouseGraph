 ...
 *
 * <p>Its own package, headless like {@code loader}, {@code saveformat}, {@code plugin}, {@code cli}
 * and {@code remote}, because of who it is for. Not {@code ui/}: there is no canvas here, and a
 * package below the UI may not reach up into it. Not {@code remote/} either — that package
 * <em>supervises</em> child processes, and this one is a child, so folding them together would make
 * the supervisor depend on the thing it supervises.
 *
 * <p>What sits below it is already built: {@code saveformat/GraphFileIO} parses the file,
