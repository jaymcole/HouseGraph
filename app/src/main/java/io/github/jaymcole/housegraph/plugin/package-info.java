 ...
 *   <li>{@link io.github.jaymcole.housegraph.plugin.RepositoryUrls} — repository-URL normalisation
 *       and matching for the daemon's optional allowlist.</li>
 *   <li>{@link io.github.jaymcole.housegraph.plugin.AutoInstallPlan} — the pure install/update/refuse
 *       decision over a dependency report. Used only by the unattended daemon; the app never
 *       installs without being asked.</li>
 ...
 * the cached jars. A library whose jar has gone missing doesn't stop a graph opening — its nodes
 * become placeholders that preserve the user's work.
 *
 * <p>In the app, every network operation is user-initiated. The one caller that fetches without
 * anyone clicking is {@link io.github.jaymcole.housegraph.plugin.PluginInstaller#apply}, and only the
