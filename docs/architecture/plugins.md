...
- **Class loading** — one shared, parent-first `URLClassLoader` over all enabled
  ...
  *(not yet built)*
- **`NodeRegistry` as an instance** — multiple scan roots, an explicit
  `ClassLoader`, index invalidation, and class → owning-library mapping.
  *(not yet built)*
- **Save format v2** — records which library each node came from, and preserves an
  unresolvable node's raw JSON so opening a graph without a library installed and
  re-saving no longer destroys those nodes. *(not yet built)*
